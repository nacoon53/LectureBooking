...
public class LectureController {
    ...
    @PostMapping
    public ResponseEntity<Lecture>createLecture(@RequestBody LectureRequestDto lecture) { //강연 등록(강연자, 강연장, 신청 인원, 강연 시간, 강연 내용 입력)
        return ResponseEntity.ok(lectureService.createLecture(lecture));
    }

    @GetMapping("/{id}/applications")
    public ResponseEntity<List<String>> getApplicationsByLectureId(@PathVariable Long lectureId) { //강연 신청자 목록(강연 별 신청한 사번 목록)
